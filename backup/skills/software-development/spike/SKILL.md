...
### Robust one-file helpers for web/content extraction spikes

For throwaway fetchers, scrapers, or extractors, bias toward a single runnable script with minimal dependencies:

- Prefer **stdlib + one ubiquitous runtime dependency** (for Python, often `requests` only) over optional parsing libraries when the task is simple enough.
- Add **page-type detection** before claiming success — e.g. distinguish `article`, `verification_page`, `unknown_page`, and `request_error`.
- Return **structured diagnostics** (`ok`, `title`, `final_url`, `status_code`, `error`) so the spike is useful even when the site blocks access.
- Support a **CLI happy path** (`python script.py <url> [--text-only]`) so the user can immediately verify behavior.
- When scraping brittle third-party pages, check multiple selectors/patterns for title/content and fail clearly instead of dereferencing `None`.

See `references/web-extraction-robustness.md` for a concrete pattern from a WeChat article extractor.
