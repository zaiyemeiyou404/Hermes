...
| `ANTHROPIC_BASE_URL` | Point Claude Code at an Anthropic-compatible gateway / third-party provider |
...
| `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` | Strip credentials from subprocesses for security |

## Third-Party Provider / Gateway Notes

Claude Code can talk to third-party deployments through an **Anthropic-compatible gateway**. Recent Claude Code releases explicitly support custom `ANTHROPIC_BASE_URL` setups and model discovery from a gateway's `/v1/models` endpoint.

Practical implications:

- If you want to use a non-Anthropic backend, prefer a gateway that exposes an **Anthropic-compatible API surface**.
- `ANTHROPIC_BASE_URL` is the primary integration point for these setups.
- A provider that is only **OpenAI-compatible** is **not** necessarily usable directly from Claude Code.
- Example durable pitfall: **DeepSeek's native API is OpenAI-compatible, not Anthropic-compatible**, so Claude Code generally needs a translation gateway/proxy in front of DeepSeek rather than pointing at DeepSeek directly.

Verification pattern:

```bash
# Check whether a gateway is advertising models in the way Claude Code expects
curl -sS "$ANTHROPIC_BASE_URL/v1/models" | head
```

If third-party setup behaves oddly, inspect Claude Code release notes / changelog for `ANTHROPIC_BASE_URL`, `/v1/models`, and `apiKeyHelper` changes before assuming the provider is unsupported.

## Cost & Performance Tips

Claude Code can run against non-first-party backends when they present an **Anthropic-compatible** API surface.

1. Use `ANTHROPIC_BASE_URL` to point Claude Code at the gateway.
2. Prefer `ANTHROPIC_API_KEY` or an `apiKeyHelper` for auth in scripted setups.
3. Expect best compatibility when the gateway exposes `/v1/models` so Claude Code can discover models.
4. If a user asks to connect an **OpenAI-compatible** backend such as DeepSeek's native API, do **not** assume direct compatibility — put an Anthropic-compatible translation proxy in front of it.

Reference: `references/third-party-gateways.md`
